package com.example.sly_mobileshop.http.persenter;


import com.example.sly_mobileshop.http.HttpMethods;
import com.example.sly_mobileshop.http.entity.GoodsEntity;
...
//                .map(new HttpResultFunc<List<GoodsEntity>>());
    ...
    public static void list(Subscriber<List<GoodsEntity>> subscriber, int catId){
        Observable<List<GoodsEntity>> observable = goodsService.list(catId)
                .map(new HttpResultFunc<List<GoodsEntity>>());//HttpResultFunc<List<GoodsEntity>>()
        toSubscribe(observable, subscriber);
    }
}
